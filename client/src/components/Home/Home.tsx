...
export const Home = () => {
  ...
  const chosenMovies = useSelector(
    (state: RootStateOrAny) => state.chosenMovies
  );
  const homeChosenMovie = useSelector(
    (state: RootStateOrAny) => state.homeChosenMovie
  );
  const modalChosenMovie = useSelector(
    (state: RootStateOrAny) => state.modalDetails
  );
          ...
          <div className="home-card-description">
            <div className="description-div">
              ...
              <div className="last-sentence">
                <h5>
                  <span>Rate: &nbsp;&nbsp;</span>
                  {homeChosenMovie.imdbRating}
                </h5>
                <button
                  onClick={moreDetails}
                  type="button"
                  data-bs-toggle="modal"
                  ref={triggeringModalRef}
                  data-bs-target={`#${modalChosenMovie.imdbID}`}
                  className="btn btn-outline-light my-btn-modal"
                >
                  More Details
                </button>
              </div>
            ...
            <div>
              <img
                className="home-img"
                src={homeChosenMovie.Poster}
                alt="chosen movie poster"
              ></img>
            </div>
          </div>
        )}
